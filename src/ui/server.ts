...
const mdWhen = (ms?: number) => (ms && Number.isFinite(ms) ? new Date(ms).toISOString().replace('T', ' ').slice(0, 16) + ' (sim)' : 'n/a')

/**
 * What the board ran against, said plainly. The page banner, the foot line
 * and the receipt all derive from this one table so they cannot disagree.
 * '{origin}' becomes board.simOrigin; '{at}' becomes ' at <origin>' when one
 * was recorded and nothing otherwise.
 */
export const MODE_COPY = {
  live: { label: 'Shared NHS-SIM world', text: 'synthetic simulator data (NHS-SIM); every action below was executed against the simulator{at} and independently re-read.' },
  local: { label: 'Local simulator stand-in', text: 'the shared NHS-SIM world is offline, so every action below was executed over HTTP against a local re-implementation of its API ({origin}) and independently re-read from it.' },
  offline: { label: 'In-process simulator stand-in', text: 'no simulator is connected; every action below was executed against Homeward\'s own in-memory stand-in and independently re-read from it.' },
  snapshot: { label: 'Replaying a saved board', text: 'no simulator is connected; approve, confirm and escalate act on the local copy only.' },
  unset: { label: 'NHS-SIM simulator', text: 'synthetic simulator data (NHS-SIM); every action below was executed against the simulator{at} and independently re-read.' },
} as const
export type BoardMode = keyof typeof MODE_COPY

export function modeStatement(board: Pick<BoardState, 'mode' | 'simOrigin'>): { mode: BoardMode; label: string; text: string } {
  const mode: BoardMode = board.mode && board.mode in MODE_COPY ? board.mode : 'unset'
  const origin = board.simOrigin ? String(board.simOrigin) : ''
  const text = MODE_COPY[mode].text.replace('{origin}', () => origin || 'origin not recorded').replace('{at}', () => (origin ? ' at ' + origin : ''))
  return { mode, label: MODE_COPY[mode].label, text }
}
...
export function buildReceipt(board: BoardState, patientId: string): string | undefined {
  ...
  L.push('')
  L.push(`**Ran against:** ${m.label}: ${m.text}`)
  L.push('')
  L.push('## Verdict')
  L.push('')
...
Idempotency key: \`${i.resolution.idempotencyKey}\``)
  ...
  L.push('---')
  L.push('')
  L.push('Synthetic simulator data (NHS-SIM); what the actions above ran against is stated under "Ran against" at the top. RECORD quotes are verbatim from the record. Final discharge decisions rest with the responsible clinician.')
  L.push('')
  return L.join('\n')
}
...
.modebar{display:flex;gap:8px;align-items:baseline;flex-wrap:wrap;padding:9px 42px;font-size:13px;line-height:1.4;border-bottom:1px solid var(--line);border-left:6px solid var(--nhs-blue);background:var(--pale);color:var(--ink)}.modebar b{white-space:nowrap}.modebar.local,.modebar.offline{background:#fff4cf;border-left-color:var(--nhs-amber)}.modebar.snapshot{background:#fbeae8;border-left-color:var(--nhs-red)}.modebar.live{background:#e9f5ed;border-left-color:var(--nhs-green)}@media(max-width:760px){.modebar{padding:9px 18px}}
...
<div class="boot" id="boot" role="status" aria-live="polite">
  <div class="card">
    ...
    <ol id="bootSteps">
      <li data-step="1"><i></i><span id="bootStep1">Joining the simulator world</span></li>
    ...
    <p class="note" id="bootMode" hidden></p>
  </div>
</div>
<div class="app" hidden>
  ...
  <div class="modebar" id="modebar" role="status"></div>
  <div class="shell">
...
// What this board ran against; the same table the server uses for the receipt, so the page cannot disagree with it.
const MODE_COPY = ${JSON.stringify(MODE_COPY)}
function modeStatement(s) {
  const mode = s && s.mode && MODE_COPY[s.mode] ? s.mode : 'unset'
  const origin = s && s.simOrigin ? String(s.simOrigin) : ''
  const text = MODE_COPY[mode].text.replace('{origin}', () => origin || 'origin not recorded').replace('{at}', () => (origin ? ' at ' + origin : ''))
  return { mode, label: MODE_COPY[mode].label, text }
}
const STEP1 = { live: 'Joining the shared NHS-SIM world', local: 'Joining the local simulator stand-in', offline: 'Starting the in-process simulator stand-in', snapshot: 'Loading the saved board', unset: 'Joining the simulator world' }
...
/** "Out of rounds: ..." in the runner's phase means the verification round cap was hit: items still awaiting are unconfirmed, not in progress. */
const roundCap = () => { const ph = String((lastState && lastState.phase) || ''); if (!/^out of rounds/i.test(ph)) return ''; const n = ph.match(/(\\d+)\\s*rounds?/i); return n ? n[1] + ' rounds' : 'the verification rounds' }
const jobTitle = (i) => {
  const k = kindOf(i)
...
const shortState = (i) => {
  ...
  if (i.state === 'awaiting_verification') { const cap = roundCap(); return cap ? 'Not confirmed after ' + cap + '; a re-run against the same world checks again' : 'Done in the ' + svc + '; verifying after time moves' }
...
function gateState(items, n) {
  ...
  if (items.some((i) => WORKING.includes(i.state))) return { cls: 'scanning', mark: '', label: items.some((i) => i.state === 'awaiting_verification') ? (roundCap() ? 'Not confirmed' : 'Verifying') : 'In progress' }
...
const hhmm = (ms) => { const d = new Date(ms); return isNaN(d) ? '--:--:--' : d.toTimeString().slice(0, 8) }
const simWhen = (ms) => { const d = new Date(ms); return ms && Number.isFinite(ms) && !isNaN(d) ? d.toISOString().slice(0, 16).replace('T', ' ') : 'n/a' }
...
function readiness(p, s) {
  ...
  if (working.length) {
    ...
    const cap = !s.busy && roundCap()
    if (cap) return R({ badge: 'Not ready: ' + open + ' outstanding', badgeCls: '', status: 'Not confirmed after ' + cap, statusCls: '', note: ['Verification rounds exhausted', working.length + ' action' + (working.length === 1 ? ' is' : 's are') + ' not yet confirmed in the owning service. Nothing was changed; re-run against the same world to check again.'], action: 'Re-run to re-check', disabled: true, text: 'The run hit its verification round cap. Unconfirmed actions stay open until a re-run re-reads them.' })
...
function whyStatus(p, s) {
  ...
  if (working.length && !s.busy && roundCap()) return { tone: 'warn', head: 'Agent stopped: ' + n(working.length, 'action') + ' not confirmed after ' + roundCap(),
    why: 'The run used all its verification rounds before the owning service showed the result.',
    next: 'Re-run against the same world to re-read them; settled items stay settled.' }
...
const wireRow = (t) => {
  ...
  return '<div class="call' + (t.ok ? '' : ' bad') + (write ? '' : ' read') + '"><div class="say"><span class="ic">' + icon + '</span>' +
    '<span class="t" title="real time">' + hhmm(t.at) + '</span><span><span class="what">' + esc(t.headline || t.action || String(t.path || '').split('?')[0]) + '</span>' +
    (t.outcome ? '<span class="out">' + esc(t.outcome) + '</span>' : '') + '</span></div>' +
    '<details class="reqdd" id="' + wireId(t) + '"><summary>Show request</summary>' +
    '<div class="head"><span class="m">' + esc(t.method) + '</span><span>' + esc(t.action || String(t.path || '').split('?')[0]) + '</span><span>' + (t.ok ? 'HTTP ' + t.status : 'FAILED ' + (t.status || '')) + '</span>' + (t.got ? '<span><span class="tag sim">sim</span> ' + esc(t.got) + '</span>' : '') + '</div>' +
...
let booted = false
const SETUP_RE = /joining|starting|stand-in|admitting|confirming|loading patient|reading the records|detect/i
function bootStep(phase) {
  const t = (phase || '').toLowerCase()
  if (/joining|starting|stand-in|connecting/.test(t)) return 1
...
function renderBoot(s) {
  const m = s ? modeStatement(s) : null
  const phase = s ? (s.phase || STEP1[m.mode]) : 'Connecting to Homeward…'
  document.getElementById('bootPhase').textContent = phase
  document.getElementById('bootStep1').textContent = STEP1[m ? m.mode : 'unset']
  const bm = document.getElementById('bootMode')
  bm.hidden = !m || m.mode === 'unset'
  bm.textContent = m && m.mode !== 'unset' ? m.label + ': ' + m.text : ''
...
function render(s) {
  ...
  const m = modeStatement(s)
  const mb = document.getElementById('modebar')
  mb.className = 'modebar ' + m.mode
  mb.innerHTML = '<b>' + esc(m.label) + ':</b> <span>' + esc(m.text) + '</span>'
  document.getElementById('foot').textContent = 'World ' + s.world + ' · sim clock ' + simWhen(s.simNow) + (s.phase ? ' · ' + s.phase : '') + ' · ' + m.label + ': ' + m.text
  if (!p) {
    document.getElementById('patientHead').innerHTML = ''
    document.getElementById('intro').innerHTML = '<div><div class="titleline"><h2>Setting up</h2><span class="top-readiness reviewing">' + esc(s.phase || STEP1[m.mode]) + '</span></div><p>The ward list appears as records load.</p></div>'
...
function renderOverlay() {
  const s = lastState; if (!s) return
  const p = patient(s); if (!p) return closeOverlay()
  ...
  if (overlayKey.startsWith('thread:')) {
    ...
    const when = r.createdAt ? simWhen(r.createdAt) : ''
  ...
  if (overlayKey.startsWith('gate:')) {
    const g = GATES[Number(overlayKey.slice(5))]; if (!g) return closeOverlay()
    const items = p.items.filter((i) => g.kinds.includes(kindOf(i)))
...
const post = async (path) => {
  try {
    ...
    if (!r.ok) console.warn('Homeward: ' + path + ' answered HTTP ' + r.status + ': ' + (await r.text()).slice(0, 200))
  } catch (e) { console.error('Homeward: ' + path + ' failed: ' + (e && e.message ? e.message : String(e))) }
  await tick(true)
}
document.addEventListener('click', async (e) => {
  ...
  if (act === 'select') { selectedPatient = el.dataset.p; letterDirty = false; lastLetterJson = ''; if (lastState) render(lastState) }
    ...
    const p = lastState && patient(lastState); if (!p) return
    const it = summaryItem(p)
    const st = document.getElementById('saveState')
    let res = null
    try { res = await fetch('/letter?patient=' + encodeURIComponent(p.patientId), { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ sections }) }) } catch (e) { res = null }
    if (res && res.ok) {
      st.textContent = it && it.state === 'verified' && !it.draftOnly ? 'Saved as an amendment; re-sending is not automated' : 'Saved, the agent sends these sections'
      letterDirty = false; lastLetterJson = ''
    } else {
      // Keep the edits: the fields stay dirty, so the next render leaves them alone.
      st.textContent = 'Save failed' + (res ? ' (HTTP ' + res.status + '): ' + (await res.text()).slice(0, 120) : ': the server did not answer') + '. Your edits are kept; try again.'
    }
    await tick(true)
  }
})
...
let pollFailed = false, lastRenderError = ''
function noteOffline() {
  const f = document.getElementById('foot')
  if (f && !/did not answer/.test(f.textContent)) f.textContent += ' · Lost contact: the Homeward server did not answer the last poll; retrying every 1.5 s.'
}
async function tick(force) {
  ...
  try {
    ...
    pollFailed = true
    if (!booted) renderBoot(null); else noteOffline()
    return
  }
  const recovered = pollFailed
  pollFailed = false
  if (!force && !recovered && text === lastText) return
  try {
    render(JSON.parse(text))
    lastText = text
    lastRenderError = ''
  } catch (e) {
    // Never swallow a render error: say it once on the console, keep polling, retry when the state changes.
    const msg = 'render failed: ' + (e && e.message ? e.message : String(e))
    if (msg !== lastRenderError) { lastRenderError = msg; console.error(msg, e) }
  }
}
setInterval(tick, 1500); tick()
</script></body></html>`

const POST_ROUTES = new Set(['/approve', '/clear-hold', '/undo-hold', '/escalate', '/letter'])
const SITES = new Set(['gp', 'hospital', 'community', 'pharmacy', 'diagnostics', 'wearables'])
/** Seven sections of up to 4000 characters each is under 30 KB; anything near this cap is not a letter. */
const LETTER_BODY_LIMIT = 256 * 1024
const plain = (res: ServerResponse, status: number, body: string): void => { res.statusCode = status; res.setHeader('content-type', 'text/plain; charset=utf-8'); res.end(body) }
...
export function startUi(board: BoardState, port = 4600, getSim?: () => SimClient | undefined): Server {
  const server = createServer((req, res) => {
    ...
    if (POST_ROUTES.has(url.pathname) && req.method !== 'POST') {
      res.setHeader('allow', 'POST')
      return plain(res, 405, `${url.pathname} takes POST only`)
    }
    if (url.pathname === '/state') {
      res.setHeader('content-type', 'application/json')
      let story: unknown = null
      try { story = computeStory(board) } catch (err) { console.error(`story: ${reason(err)}`) }
      res.end(JSON.stringify({ ...board, story }))
    } else if (url.pathname === '/undo-hold') {
      const ok = undoHold(board, url.searchParams.get('item') ?? '', 'Demo clinician')
      plain(res, ok ? 200 : 409, ok ? 'reinstated' : 'cannot undo')
      ...
      if (!SITES.has(site)) return plain(res, 400, 'unknown site')
      if (!patient || !id) return plain(res, 400, 'patient and id are required')
      let sim: SimClient | undefined
      try { sim = getSim?.() } catch (err) { return plain(res, 503, `simulator not available: ${reason(err).slice(0, 200)}`) }
      if (!sim) return plain(res, 503, 'simulator not connected (fallback mode)')
      const client = sim
      // Start inside a promise so a client that throws synchronously lands in the catch below, not on the process.
      Promise.resolve().then(() => client.siteView(site as Parameters<SimClient['siteView']>[0], { patient, limit: 300 })).then((view) => {
        const r = ((view.resources ?? []) as Array<{ id?: string }>).find((x) => x.id === id)
        if (!r) return plain(res, 404, 'not in the ' + site + ' record')
        res.setHeader('content-type', 'application/json')
        res.end(JSON.stringify(r))
      }).catch((err) => plain(res, 502, reason(err).slice(0, 200)))
    } else if (url.pathname === '/clear-hold') {
      const ok = clearHold(board, url.searchParams.get('item') ?? '', 'Demo clinician')
      plain(res, ok ? 200 : 404, ok ? 'cleared' : 'not found')
    } else if (url.pathname === '/approve') {
      const n = approveAll(board, 'Demo coordinator', url.searchParams.get('patient') || undefined)
      plain(res, 200, String(n))
    } else if (url.pathname === '/receipt') {
      const patientId = url.searchParams.get('patient') ?? ''
      const md = buildReceipt(board, patientId)
      if (!md) return plain(res, 404, 'unknown patient')
      res.setHeader('content-type', 'text/markdown; charset=utf-8')
      res.setHeader('content-disposition', `attachment; filename="homeward-receipt-${patientId.replace(/[^A-Za-z0-9._-]/g, '_')}.md"`)
      res.end(md)
    } else if (url.pathname === '/letter') {
      const chunks: Buffer[] = []
      let size = 0, tooBig = false
      req.on('data', (c: Buffer) => {
        if (tooBig) return
        size += c.length
        if (size > LETTER_BODY_LIMIT) { tooBig = true; plain(res, 413, `letter body over ${LETTER_BODY_LIMIT} bytes; nothing saved`); return }
        chunks.push(c)
      })
      req.on('error', () => { /* the client went away mid-body; there is nobody left to answer */ })
      req.on('end', () => {
        if (tooBig) return
        let parsed: unknown
        try { parsed = JSON.parse(Buffer.concat(chunks).toString('utf8')) } catch { return plain(res, 400, 'invalid JSON body; nothing saved') }
        const sections = parsed && typeof parsed === 'object' && !Array.isArray(parsed) ? (parsed as { sections?: unknown }).sections : undefined
        if (!sections || typeof sections !== 'object' || Array.isArray(sections)) return plain(res, 400, 'expected {"sections": {...}}; nothing saved')
        const ok = saveLetter(board, url.searchParams.get('patient') ?? '', sections as Record<string, unknown>, 'Demo clinician')
        plain(res, ok ? 200 : 404, ok ? 'saved' : 'not found')
      })
    } else if (url.pathname === '/escalate') {
      prepareEscalation(board, url.searchParams.get('item') ?? '')
        .then((ok) => plain(res, ok ? 200 : 404, ok ? 'escalated' : 'not found'))
        .catch((err) => plain(res, 500, `escalation failed: ${reason(err).slice(0, 200)}`))
    } else {
