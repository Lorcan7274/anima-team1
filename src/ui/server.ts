...
function renderOverlay() {
  ...
  if (overlayKey === 'plan') {
    ...
    const wontLine = (i) => '<li><b>' + esc(i.state === 'clinical_hold' ? 'Clear the clinical hold' : shortTitle(i.title)) + '</b><span>' + esc(i.state === 'clinical_hold' ? 'only the clinician can, by recording the review' : i.humanReason || 'an outside decision the agent must not make') + '</span></li>'
    const summary = '<div class="wontwill">' +
      ...
      '<div class="wont"><h4>The agent will not</h4><ul>' +
        '<li><b>Start anything before you approve</b><span>nothing below runs until you press Approve</span></li>' +
        humans.map(wontLine).join('') +
        '<li><b>Make the discharge decision</b><span>that stays with the responsible clinician</span></li>' +
        '<li><b>Invent clinical facts</b><span>letters and requests quote the record; the drafting source is shown on each</span></li>' +
      '</ul></div></div>' +
