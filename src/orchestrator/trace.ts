...
const KIND_LABEL: Record<string, string> = {
  ...
  appointment: 'Appointment', 'appointment-session': 'Appointment session',
}
  ...
  switch (t) {
    ...
    case 'process_document': {
      ...
      const DOC: Record<string, string> = {
        send: 'Sent the discharge summary to the GP', file: 'Filed the document', review: 'Reviewed the document',
        assign: 'Assigned the document', annotate: 'Annotated the document',
      }
      return DOC[cmd] ?? `${capital(cmd)} the document`
    }
    ...
    case 'cancel_appointment': return 'Cancelled the appointment'
    case 'create_appointment_session': return `Opened a${body.mode ? ` ${words(body.mode)}` : 'n'} appointment session`
    case 'book_appointment': return `Booked an appointment${title ? `: ${title}` : ''}`
...
export function outcomeFor(body: Body, reply: Reply | undefined, ok: boolean, httpStatus?: number, error?: string): string {
  if (!ok) {
    const reason = plain(error ?? summarise(reply))
...
function kindFromType(t?: string): string {
  ...
  if (t === 'create_appointment_session') return 'appointment-session'
  if (/appointment/.test(t)) return 'appointment'
  return ''
}

/** An error page (HTML from a proxy or gateway) reduced to its words. */
function plain(reason: string): string {
  return reason.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 200)
}
...
export function annotate(t: TraceEntry): TraceEntry {
  ...
  t.outcome = t.ok ? undefined : `Failed${t.status ? ` (HTTP ${t.status})` : ''}${t.error ? `: ${plain(t.error)}` : ''}`
  return t
}
